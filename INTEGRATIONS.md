...
| `TextEntry/WorkoutCSVParser` | Deterministic Hevy / Strong tables. CSV never goes through Foundation Models. Hevy warmup rows are dropped; drop/failure sets stay and are tagged. A Hevy `failure` row with 0 reps (or no load) is kept. A `set_index` reset starts a new exercise so logging the same lift twice is not interleaved. `superset_id` becomes a set note. RPE maps to `SetEntry.difficulty`. Description / Workout Notes / exercise notes / set notes compose onto the session and sets. `end_time` or Strong `Duration` (`60m`) become session `endedAt` and ledger `durationSeconds`. EU/Android Strong (`;` delimiter, `Weight (kg)`, decimal commas) parses. Strong `Distance` uses km or mi from the preferred weight unit, not metres. Trailing `(Barbell)` equipment suffixes are stripped on the **draft name** after grouping, so `Squat (Barbell)` and `Squat (Dumbbell)` stay two exercises. |
...
| `TextEntry/WorkoutTextEntryView` | Primary **Add** tab plus a reusable sheet handoff for Scan. File picker is `.txt` / `.csv` (not JSON). Multi-file Hevy/Strong headers merge. Complete notation uses the deterministic fast path; unresolved prose uses Apple Intelligence when available. N==1 → editable review; N>1 → Health-style batch list with per-row library / new / weak-match counts. Weak matches stay suggestions and default to create-new. Review shows rest, RPE, and notes before save. |
| `Intents/ReviewWorkoutTextIntent` + `ReviewWorkoutFileIntent` | Stages `PendingTextImport` and opens Add directly. If a draft already exists, the user chooses append, replace, or keep current. |
...
The deterministic parser also reads `RPE 8` / `rpe9` / `@RPE 8` on typed and scanned lines (not bare `@ 8`, which stays weight). Numbered session labels are titles, not exercises. Scan review can reorder exercises like Paste or Type. Blank RPE/rest in review save as 8 / the exercise default.
