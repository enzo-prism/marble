...
NO_WATCH=0
FORCE_GITHUB=0
EVIDENCE_RUN_ID="${RELEASE_EVIDENCE_RUN_ID:-}"
UPLOAD_RUN_ID="${RELEASE_UPLOAD_RUN_ID:-}"
DEVICE_SIGNOFF_PATH="${RELEASE_DEVICE_SIGNOFF:-}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --github) FORCE_GITHUB=1; shift ;;
    --evidence-run) EVIDENCE_RUN_ID="${2:-}"; shift 2 ;;
    --upload-run) UPLOAD_RUN_ID="${2:-}"; shift 2 ;;
    --device-signoff) DEVICE_SIGNOFF_PATH="${2:-}"; shift 2 ;;
...
watch_workflow() {
  ...
  for i in $(seq 1 30); do
    run_id="$(gh run list --repo "$MARBLE_GITHUB_SLUG" --workflow "$workflow_file" --limit 10 \
      ...
      --jq ".[] | select(.headSha==\"${sha}\" and .createdAt>=\"${requested_at}\") | .databaseId" 2>/dev/null | head -n 1 || true)"
    if [[ -n "$run_id" ]]; then
      gh run watch "$run_id" --repo "$MARBLE_GITHUB_SLUG" --exit-status || {
        echo "error: workflow failed or its outcome could not be verified. Run ID: ${run_id}" >&2
        return 1
      }
      return 0
    fi
...
dispatch_or_tag() {
  ...
  if [[ -z "$branch" || "$branch" == "HEAD" ]]; then
    echo "Detached HEAD: workflow_dispatch needs a branch/tag name"
    branch=""
  fi
  ...
  if [[ "$status" == "204" ]]; then
    ...
    rm -f "$tmp"
    watch_workflow "$workflow_file" "$sha" "$requested_at"
    return $?
  fi

  if [[ "$status" != "000" ]]; then
    echo "workflow_dispatch returned HTTP ${status}"
    if [[ -s "$tmp" ]]; then
      head -c 500 "$tmp"; echo
    fi
  fi
  ...
  if [[ -z "$tag" || "$DRY_RUN" -eq 1 ]]; then
    echo "error: dispatch was not accepted; this action has no tag fallback. No release was started." >&2
    return 1
  fi
...
appstore_dispatch_inputs() {
  local action="$1" signoff=""
  if [[ "$DRY_RUN" -eq 0 ]]; then
    if [[ ! "$EVIDENCE_RUN_ID" =~ ^[0-9]+$ || ! "$UPLOAD_RUN_ID" =~ ^[0-9]+$ || ! -f "$DEVICE_SIGNOFF_PATH" ]]; then
      echo "error: production dispatch requires --evidence-run ID --upload-run ID --device-signoff FILE (or matching RELEASE_* environment variables)" >&2
      return 1
    fi
    signoff="$(jq -ce 'if type == "object" then . else error("Device signoff must be a JSON object") end' "$DEVICE_SIGNOFF_PATH")"
  fi
  jq -n --arg action "$action" --arg version "$VERSION" --arg build "$BUILD_ID" \
    --arg evidence "$EVIDENCE_RUN_ID" --arg upload "$UPLOAD_RUN_ID" --arg signoff "$signoff" \
    --argjson dry_run "$([[ "$DRY_RUN" -eq 1 ]] && echo true || echo false)" \
    '{action: $action, version: $version, build_id: $build, confirm: $action, dry_run: $dry_run,
      evidence_run_id: $evidence, upload_run_id: $upload, device_signoff: $signoff}'
}
...
run_local_api() {
  ...
  marble_export_asc_ci
  if [[ -n "$DEVICE_SIGNOFF_PATH" ]]; then export RELEASE_DEVICE_SIGNOFF="$DEVICE_SIGNOFF_PATH"; fi
...
case "$COMMAND" in
  ...
  appstore-submit)
    ...
    if [[ "$FORCE_GITHUB" -eq 0 ]] && marble_asc_env_ready; then
      extra=(submit --version "$VERSION" --confirm)
      [[ -n "$BUILD_ID" ]] && extra+=(--build "$BUILD_ID")
      [[ "$DRY_RUN" -eq 1 ]] && extra+=(--dry-run)
      run_local_api "${extra[@]}"
    else
      inputs="$(appstore_dispatch_inputs submit)"
      dispatch_or_tag "release-appstore.yml" "$inputs" ""
    fi
    ;;
  appstore-release)
    if [[ "$CONFIRM" -ne 1 ]]; then
    ...
    if [[ "$FORCE_GITHUB" -eq 0 ]] && marble_asc_env_ready; then
      ...
      inputs="$(appstore_dispatch_inputs release)"
      dispatch_or_tag "release-appstore.yml" "$inputs" ""
    fi
    ;;
  *)
