...
> `IN_BETA_TESTING` internally, and is now `WAITING_FOR_REVIEW` under submission
> `ce4a0d8a-...` with manual release. The superseded build 60 submission is
> `COMPLETE`; public production remains 2.3 build 56. Physical-device,
...
> | Phase | State |
> |---|---|
> | — build 61 UI and accessibility candidate (2026-08-24) | **Current TestFlight and App Review candidate:** refines the AI-first Add hierarchy, adaptive light/dark surfaces, Calendar density, Add Set action semantics, progress-photo crop controls, grayscale Progress cues, 44-point targets, maximum Larger Text, centered iPad portrait/landscape layouts, and compact-width iPhone landscape. A refreshed iPad matrix found and fixed reused Calendar grid identities that hid January 1–6 in landscape; phone-landscape coverage then caught and fixed an oversized width-derived Calendar height. The adaptive theme helpers are nonisolated to avoid a Swift concurrency runtime trap. Snapshot recording now truly overwrites baselines in the dedicated record plan. No schema change (still V6). Exact uploaded `main` source `9e8346f6cad4683991a78fbaf223baaf01e9f068`; ASC build `bbea8736-b964-4175-8c9c-b140ebcea4c1` is `VALID`, internal `IN_BETA_TESTING`, external `READY_FOR_BETA_SUBMISSION`; exact-main CI `32771324947` and TestFlight workflow `32772039189` passed; strict TestFlight validation is 0/0/0/0. Build 61 is attached to App Store 2.4 and submission `ce4a0d8a-f5ea-4d1e-9463-03937e467343` is `WAITING_FOR_REVIEW` under manual release. Public production remains 2.3 build 56. |
> | — build 60 App Review replacement (2026-08-24) | **Superseded release fix:** restores a persistent Start/open workout action on the default Add tab so a workout can be started when no session accessory exists; PR celebration resolves the exact library UUID approved in import review. No schema change (still V6). Exact uploaded `main` source `b287d7238494799818db5947524a5cc05b9c8a9c`; ASC build `ef651ca3-451f-468b-903a-1239bcf6dc39` remains `VALID` and internal `IN_BETA_TESTING`. Exact-main CI `32704746418`, TestFlight workflow `32705379775`, and strict validation passed. Its App Review submission `5bd874ad-3619-4521-9231-fa45ee18a4b0` is now `COMPLETE` after build 61 replaced it. It was never public; storefront production remains 2.3 build 56. |
> | — build 59 TestFlight / superseded App Review (2026-08-24) | **AI-first Add:** Add replaces Train as the default tab and directly hosts the paste-or-type composer. On-device Apple Intelligence handles prose when available; deterministic notation/CSV parsing remains the fast path and fallback. Structured review precedes save, with draft recovery, Shortcut handoff, exact exercise identity, stable unresolved-line retry, and active-session access from the tab-bar accessory. No schema change (still V6). Feature source `c8200e2`; uploaded merge `eead033`; build `3235fff4-...` remains a valid TestFlight build. Its old submission `2233970e-...` was canceled and is `COMPLETE` after build 60 replaced it. |
...
> | — App Store (verified 2026-08-24) | **Train / Log / Progress IA shipped publicly as 2.3 build 56:** three tab-bar destinations; Calendar + Supplements are Log modes; one-tap complete-set on planned rows with history (`SetLogging`); medium morphing Log Set sheet; session accessory; lock-screen widget drops two-digit streaks. No schema change (still V6). Shipped application source: PR #19 merge `c0cef9e`. |
...
> | — main (2026-07-30) | **Unreleased free-form notation wave** (from a real failing pole-vault note): deterministic parser learned hyphenated units ("20-meter"), en-dash rep ranges ("8–10"), intensity-% noise, distance sets ("4×20m"), spec-first lines, "5 by 5", single/double/triple, word weight units, name-filler stripping; arbiter gained coverage scoring + colon-duration tokens; FM instructions gained distance-vs-reps and session-vs-per-set rules + structured-pass retry. Eval: **25/25 (100%)** stable. |
> | — main (2026-07-29) | **Unreleased on-device parsing quality overhaul**: FoundationModels pipeline rebuilt — count-based `@Generable` schema (code expands sets), notation-rewrite second pass, `WorkoutDraftArbiter` scoring model drafts against the source text, permissive-transformation guardrails + one retry (default guardrails refuse benign gym prose), greedy sampling, prewarm. 24-case eval corpus + opt-in live eval: 50% → 88%. |
