...
> The AI-first Add release is 2.4 build 59 at application-source commit
> `c8200e2`, built from merge `eead033`. TestFlight build 59 is `VALID` and
> internal; App Store 2.4 is `WAITING_FOR_REVIEW` with manual release. Local
> branch `codex/build-60-release-fixes` is a fixed build 60 replacement candidate;
> its source SHA and App Store Connect build ID are pending, and it has not replaced
> build 59 anywhere. Physical-device,
...
> | Phase | State |
> |---|---|
> | — build 60 replacement candidate (2026-08-24) | **Release fixes, local only:** restores a persistent Start/open workout action on the default Add tab so a workout can be started when no session accessory exists; PR celebration now resolves the exact library UUID approved in import review and stays quiet for explicit create-new exercises, preventing false PRs when duplicate names exist. No schema change (still V6). Source `BUILD_60_SOURCE_SHA_PENDING`; ASC build `BUILD_60_ASC_ID_PENDING`. Build 59 remains `WAITING_FOR_REVIEW` under manual release; keep that submission intact unless an exact verified build 60 replacement is explicitly approved. |
> | — build 59 TestFlight / App Review (2026-08-24) | **AI-first Add:** Add replaces Train as the default tab and directly hosts the paste-or-type composer. On-device Apple Intelligence handles prose when available; deterministic notation/CSV parsing remains the fast path and fallback. Structured review precedes save, with draft recovery, Shortcut handoff, exact exercise identity, stable unresolved-line retry, and active-session access from the tab-bar accessory. No schema change (still V6). Feature source `c8200e2`; uploaded merge `eead033`; build `3235fff4-...` is `VALID` internally and App Store 2.4 is `WAITING_FOR_REVIEW`. |
