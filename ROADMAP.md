...
> replacement build 60 was uploaded from exact `main` source `b287d72`, is
> `VALID` / `IN_BETA_TESTING` internally, and is now `WAITING_FOR_REVIEW` under
> submission `5bd874ad-...` with manual release. The old build 59 submission was
> canceled and is `COMPLETE`; public production remains 2.3 build 56. Physical-device,
...
> | Phase | State |
> |---|---|
> | — build 61 UI and accessibility candidate (2026-08-24) | **Pending merge and upload:** refines the AI-first Add hierarchy, adaptive light/dark surfaces, Calendar density, Add Set action semantics, progress-photo crop controls, grayscale Progress cues, 44-point targets, maximum Larger Text, centered iPad portrait/landscape layouts, and compact-width iPhone landscape. A refreshed iPad matrix found and fixed reused Calendar grid identities that hid January 1–6 in landscape; phone-landscape coverage then caught and fixed an oversized width-derived Calendar height. The adaptive theme helpers are nonisolated to avoid a Swift concurrency runtime trap. Snapshot recording now truly overwrites baselines in the dedicated record plan. No schema change (still V6). Build 60 remains the verified TestFlight/App Review source until exact-main CI and build 61 live readback complete. |
> | — build 60 App Review replacement (2026-08-24) | **Release fixes:** restores a persistent Start/open workout action on the default Add tab so a workout can be started when no session accessory exists; PR celebration now resolves the exact library UUID approved in import review and stays quiet for explicit create-new exercises, preventing false PRs when duplicate names exist. No schema change (still V6). Exact uploaded `main` source `b287d7238494799818db5947524a5cc05b9c8a9c`; ASC build `ef651ca3-451f-468b-903a-1239bcf6dc39` is `VALID`, internal `IN_BETA_TESTING`, external `READY_FOR_BETA_SUBMISSION`. Exact-main CI `32704746418` and TestFlight workflow `32705379775` passed; strict TestFlight validation is 0 errors / 0 warnings / 0 blockers / 0 informational findings. Build 60 is attached to App Store 2.4 and submission `5bd874ad-3619-4521-9231-fa45ee18a4b0` is `WAITING_FOR_REVIEW` under manual release. It is not approved or public; storefront production remains 2.3 build 56. |
